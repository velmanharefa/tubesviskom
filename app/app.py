...
MODEL_PATH = "best_modelSDG100.pt"  # Path ke model YOLOv11 Anda
...
if uploaded_file is not None:
    ...
    # Tangani gambar dengan transparansi (RGBA)
    if image.mode == "RGBA":
...
if st.session_state['image'] is not None and st.button("Mulai Prediksi"):
    with st.spinner("Memproses gambar, mohon tunggu..."):
        time.sleep(2)  # Simulasi loading untuk user experience
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Tambahkan bounding box ke gambar
